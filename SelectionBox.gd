...
onready var direct_space_state: PhysicsDirectSpaceState = get_parent().get_node("Person").get_world().direct_space_state
...
func _unhandled_input(event):
	if Globals.cursor_state == Globals.CursorState.Select:
		...
		if event.is_action_released("left_click"):
			...
			var num_selected = get_parent().get_node("Person").select_units(get_rect(), get_tree().get_nodes_in_group("surface")[0].player_team)
			if num_selected < 1:
				...
				if not raycast_result.empty() and "IS_UNIT" in raycast_result.collider.get_parent():
					...
					if unit in get_parent().get_node("Person").player_team.units:
